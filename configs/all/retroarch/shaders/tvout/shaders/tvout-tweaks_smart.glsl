...
// automatic signal resolution
// adjusts signal resolution according to the input resolution
// only works with "TVOut Composite Enable" off
// 1 = exact resolution match
// 2 = adjust resolution to blend dithering and reduce rainbow artifacts when appropriate
#pragma parameter TVOUT_AUTO "TVOut Auto-Resolution" 1.0 0.0 2.0 1.0 // on by default
...
uniform COMPAT_PRECISION float TVOUT_AUTO;
...
#define TVOUT_AUTO 1.0
#define TVOUT_COMPOSITE_CONNECTION 0.0
#define TVOUT_TV_COLOR_LEVELS 0.0
...
#define VAL(tempColor) \
   ...
   else if (TVOUT_AUTO == 1.0) \
      tempColor += (c*STU(X,(InputSize.x*oneI))); \
   else if (TVOUT_AUTO == 2.0 && InputSize.x >= 333.0) \
      tempColor += (c*STU(X,((InputSize.x*0.65)*oneI))); \
   else if (TVOUT_AUTO == 2.0 && InputSize.x <= 332.0 && InputSize.x >= 304.0) \
      tempColor += (c*STU(X,(InputSize.x*(216.0/InputSize.x)*oneI))); \
   else \
