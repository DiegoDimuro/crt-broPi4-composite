/*
    ...
    An ultra light scanline shader with hi-res detection
    by RiskyJumps
...
#pragma parameter imageSize "Simulated Image Height" 224.0 144.0 448.0 1.0
#pragma parameter autoscale "Automatic Scale" 0.0 0.0 1.0 1.0
#pragma parameter detecthires "Detect Hi-Res" 1.0 0.0 2.0 1.0
...
uniform COMPAT_PRECISION float amp, phase, lines_black,  lines_white,
         imageSize,  autoscale,  detecthires;
#else
#define amp  1.25
#define phase  0.5
...
#define detecthires 1.0

#endif

void main()
{
    float scale = imageSize; if (autoscale == 1.0) scale = InputSize.y; if (autoscale == 1.0 && detecthires == 2.0) scale = InputSize.y / 2.0;
    ...
    FragColor = vec4(color.xyz, 1.0); if (detecthires == 1.0 && InputSize.y >= 400.0) FragColor = vec4(COMPAT_TEXTURE(Source, vTexCoord).xyz, 1.0);
}
